...
void DeviceManager::Unload()
{
	...
	for (auto agent : midiAgents) {
		blog(LOG_DEBUG, "Unloading Midi Device %s",
		     agent->get_midi_input_name().toStdString().c_str());
...
QStringList DeviceManager::GetPortsList()
{
	...
	for (int i = 0; i < portCount; i++) {
		ports.append(QString::fromStdString(
			rtmidi::midi_in().get_port_name(i)));
	}
	return ports;
}
